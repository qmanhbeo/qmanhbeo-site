...
**Current session note - 2026-05-10 temporary night lock:** Added a temporary top-level `WORLD_TIME_LOCK` override in `EnvironmentManager` and set it to `NIGHT` so `/world` remains in permanent night/dusk art direction while daytime building assets are unfinished. This lock is centralized (not scattered), keeps the existing time manager, day/night calculations, transitions, sky/lighting, and moon/stars systems intact, and can be reverted by setting `WORLD_TIME_LOCK` to `undefined`.

**Current session note - 2026-05-17 sky visibility root-cause fix:** Confirmed moon/star placement was initialized before camera follow + responsive zoom were applied, so sky objects started from stale camera scroll and could load clipped near the top/right on desktop until a resize/zoom event forced repositioning. Applied the minimal safe fix by moving `EnvironmentManager.create()` to run after player follow and initial camera zoom/deadzone setup in `WorldScene`, without changing art layers, time-state behavior, or introducing new positioning systems.
...
- [x] 2026-05-10: Added a temporary centralized world time lock in `game/EnvironmentManager.ts` (`WORLD_TIME_LOCK = "NIGHT"`) to force visual coherence while daytime building assets are unfinished, without deleting or refactoring the existing day/night infrastructure.
...
- [x] Supplied ground/path/decor visual slice — runtime-normalized 64x64 ground/path textures, transformed vertical/corner variants, deterministic grass variation, and sparse no-collision grass decorations
