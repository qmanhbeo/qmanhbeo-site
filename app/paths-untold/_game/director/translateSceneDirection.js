...
function beatInstruction(plan) {
  ...
  switch (plan.beat) {
    ...
    case 'escalate':
      return `Escalate the pressure with a visible consequence or tightening constraint, without resolving it.${modifierText}`;
    case 'hold':
      return `Hold tension steady while giving the player new texture, evidence, or positioning.${modifierText}`;
    case 'partial_release':
      return `Offer partial relief: reduce immediate strain while making meaningful progress toward the thread objective.${modifierText}`;
    case 'resolve':
      return `Resolve this thread decisively and show the immediate consequence of that resolution.${modifierText}`;
    case 'aftermath':
      return 'Deliver the queued aftermath: show consequences, relationship change, or the emotional cost of the recent resolution.';
    default:
